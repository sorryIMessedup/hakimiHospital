		...
		<div class="hospital_login-box">
			...
			<div class="hospital_login-rightBox">
				...
				<div class="hospital_login-username">
					<svg t="1679019720900" class="icon" viewBox="0 0 1024 1024" ref="svgUsername" version="1.1"
						xmlns="http://www.w3.org/2000/svg" p-id="2740">
						<path
				...
				<div class="hospital_login-password">
					<svg t="1679020061270" class="icon" viewBox="0 0 1024 1024" ref="svgPassword" version="1.1"
						xmlns="http://www.w3.org/2000/svg" p-id="4319">
						<path
			...
			if (this.type === "admin") {
				this.loader.post("/users/login", {
					...
					if (val.status == 200) {
						...
						this.$message.error('登陆失败，用户名或密码错误1');
					}
				}, err => {
					this.$message.error('登陆失败，用户名或密码错误2');
					console.log(err);
				}).finally(() => {
					this.username = "";
					this.password = "";
				...
				this.loader.post("/users/login", {
					...
					if (val.status === 200) {
						...
						this.$message.error('登陆失败，用户名或密码错误3');
					}
				}, err => {
					this.$message.error('登陆失败，用户名或密码错误4');
					console.log(err);
				}).finally(() => {
					this.username = "";
					this.password = "";
...
.hospital_login-container {
	...
	.hospital_login-box {
		...
		.hospital_login-rightBox {
			...
			.hospital_login-username,
			.hospital_login-password {
				display: flex;
				align-items: center;
				height: 22px;
				width: 100%;
