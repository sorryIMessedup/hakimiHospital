	...
	<div id="picture_editor">
		...
		<div class="pic_video_button">
			<template v-if="component_type != 2">
				...
				<template v-else>
					<template v-if="component_type == 1">
						<p style="color:#909399; width:405px">请导入图片资源</p>
					</template>
					<template v-else>
						<p style="color:#909399; width:405px">不存在图片资源</p>
					</template>
				</template>

				<el-upload :style="(image_url == null) ? { 'margin-left': '30px' } : { 'margin-left': '30px', 'margin-top': '160px' }"
					:file-list="fileList" class="upload-demo" action="#" :on-preview="handlePreview" :http-request="uploadFile"
